...
async def get_connected_components(
    min_size: int = Query(
        2,
        description="Minimum size of component to report"
    )
):
    """
    Find connected components without relying on APOC.

    Uses native Cypher traversal to collect all non-Document nodes
    reachable from each starting node, then groups nodes into components
    using the minimum elementId as a stable component identifier.
    """
    try:
        if min_size < 1:
            raise HTTPException(
        ...
        WITH
            min(elementId(member)) AS component_id,
        ...
        RETURN
            ...
            [
                member IN component_nodes |
                {
                    ...
                    value:
                        CASE
                            WHEN member:Person THEN
                                properties(member)['person_name']

                            WHEN member:Phone THEN
                                properties(member)['phone_number']

                            WHEN member:Vehicle THEN
                                properties(member)['registration_number']

                            WHEN member:BankAccount THEN
                                properties(member)['account_number']

                            WHEN member:Organization THEN
                                properties(member)['name']

                            WHEN member:Location THEN
                                coalesce(
                                    properties(member)['name'],
                                    properties(member)['address']
                                )

                            WHEN member:Case THEN
                                coalesce(
                            ...
                            WHEN member:Document THEN
                                coalesce(
                                    properties(member)['document_id'],
                                    properties(member)['title']
                                )

                            WHEN member:Email THEN
                                coalesce(
                                    properties(member)['email_address'],
                                    properties(member)['name']
                                )
...
async def get_clustering_coefficient(entity_id: str):
    ...
    Measures how closely the neighbors of a node are connected to each
    other.
    """
    try:
        query = """
        ...
        WITH
            n,
            count(DISTINCT f) AS connections_between_neighbors
        ...
        RETURN
            ...
            CASE
                WHEN n:Person THEN
                    properties(n)['person_name']

                WHEN n:Phone THEN
                    properties(n)['phone_number']

                WHEN n:Vehicle THEN
                    properties(n)['registration_number']

                WHEN n:BankAccount THEN
                    properties(n)['account_number']

                WHEN n:Organization THEN
                    properties(n)['name']
            ...
            CASE
                ...
                ELSE
                    toFloat(2 * connections_between_neighbors) /
        ...
        return {
            ...
            "connections_between_neighbors": record[
                "connections_between_neighbors"
            ],
            "clustering_coefficient": record[
